...
class MACAEnv(gym.Env):
    ...
    def _take_action(self, actions):
        ...
        for agent_index, agent in enumerate(self.agents):
            ...
            all_actions[agent_index, :] = agent.find_next_action(dict_obs, actions)
    ...
    def _check_for_collisions(self):
        ...
        for i, j in agent_pairs:
            if self.agents[i].pos_global_frame is not None and  self.agents[j].pos_global_frame is not None:
                ...
                if dist_btwn <= combined_radius:
                    ...
                    collision_with_agent[i] = False
                    collision_with_agent[j] = False
        agent_inds = list(range(len(self.agents)))
        ...
        for obstacle in self.obstacles:
            for j in agent_inds:
                ...
                if dist_btwn <= combined_radius:
                    ...
                    collision_with_obstacle[j] = False

        return collision_with_obstacle, collision_with_agent, collision_with_wall, entered_norm_zone, dist_btwn_nearest_agent, dist_to_nearest_obstacle
