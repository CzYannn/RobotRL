...
class UnicycleDynamics(Dynamics):
    ...
    def step(self, action, dt):
        ...
        cmb_array = np.concatenate([np.array([[self.agent.max_x, self.agent.max_y]]), self.agent.pos_global_frame[np.newaxis,:]], axis=0)
        self.agent.pos_global_frame = np.min(cmb_array, axis=0)
        cmb_array = np.concatenate([np.array([[self.agent.min_x, self.agent.min_y]]), self.agent.pos_global_frame[np.newaxis,:]], axis=0)
