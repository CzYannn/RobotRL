...
class Agent(object):
    def __init__(self, name, radius, pref_speed,
        initial_heading, policy, dynamics_model, sensors, id, start_pos = None, goal_pos = None, group=0):
        self.id = id
        self.group = group
        self.name = name
        self.radius = radius
        ...
        self.max_heading_change = np.pi / 18
...
#        self.time_remaining_to_reach_goal = max(self.time_remaining_to_reach_goal, self.dt_nominal)
    ...
    def find_next_action(self, dict_obs, actions):
    ...
    def take_action(self, action):
        ...
        if self.obj is None: # update_from_cal
            ...
            self.obj.pubTwist(action, self.dt_nominal)
